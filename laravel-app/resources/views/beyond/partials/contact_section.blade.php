...
<section id="contact"
         class="py-10 bg-gray-50 scroll-mt-24"
         x-data="{ open: (typeof window !== 'undefined' && window.location.hash === '#contact') }"
         x-init="$watch('open', function (v) { if (v && window.lucide) { window.lucide.createIcons(); } })">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div class="text-center">
            <button type="button"
                    @click="open = !open; if (open) { history.replaceState(null, '', '#contact'); } else { history.replaceState(null, '', window.location.pathname + window.location.search); }"
                    class="inline-flex items-center gap-3 text-3xl md:text-4xl font-extrabold text-brand-gold hover:text-black transition-colors">
                {{ \App\Support\SiteContent::text('contact.heading', __('site.contact.heading')) }}
                <i data-lucide="chevron-down" class="w-8 h-8 transition-transform" :class="open ? 'rotate-180' : ''"></i>
            </button>
        </div>

        <div x-show="open" x-cloak x-transition class="mt-10">
