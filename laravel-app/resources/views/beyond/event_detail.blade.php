...
@section('title', ($pub->public_title ?: $event->name) . ' | ' . __('site.nav.events'))
@section('meta_description', $pub->public_summary ?: __('site.events.details'))
...
<section class="relative bg-brand-navy text-white overflow-hidden">
    ...
    <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
        <p class="text-brand-gold font-semibold tracking-wide uppercase text-sm mb-3">{{ __('site.events.types.'.$event->event_type) }}</p>
        ...
        @if($publicStatus)
            <span class="inline-block text-sm font-semibold px-4 py-1 rounded-full {{ $statusColors[$publicStatus] ?? 'bg-white/20 text-white' }}">
                {{ __('site.events.statuses.'.$publicStatus) }}
            </span>
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div class="lg:col-span-2">
                @if($pub->public_announcement)
                    <div class="mb-8 p-4 bg-amber-50 border-l-4 border-brand-gold rounded-r-lg">
                        <p class="font-semibold text-brand-blue mb-1">{{ __('site.events.announcement') }}</p>
            ...
            <div class="space-y-6">
                <div class="bg-gray-50 rounded-xl p-6 border border-gray-200">
                    <h3 class="font-bold text-brand-blue mb-4">{{ __('site.events.details') }}</h3>
                    <ul class="space-y-4 text-sm">
                        @if($pub->show_event_time && $event->event_start_at)
                            <li class="flex gap-3">
                                <i data-lucide="calendar" class="w-5 h-5 text-brand-blue shrink-0"></i>
                                <div>
                                    <p class="font-medium text-gray-900">{{ __('site.events.starts') }}</p>
                        ...
                        @if($pub->show_event_time && $event->event_end_at)
                            <li class="flex gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900">{{ __('site.events.ends') }}</p>
                        ...
                        @if($pub->show_setup_info && $event->setup_start_at)
                            <li class="flex gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900">{{ __('site.events.setup') }}</p>
                        ...
                        @if($pub->public_venue || $pub->public_location)
                            <li class="flex gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900">{{ __('site.events.location') }}</p>
                        ...
                        @if($pub->public_contact_name || $pub->public_contact_phone)
                            <li class="flex gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900">{{ __('site.events.contact') }}</p>
...
<section class="py-12 bg-gray-50 border-t">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-bold text-brand-blue mb-8">{{ __('site.events.related') }}</h2>
...
<div class="py-8 text-center">
    <a href="{{ url('/events') }}" class="text-brand-blue font-semibold hover:underline">← {{ __('site.events.all') }}</a>
</div>
