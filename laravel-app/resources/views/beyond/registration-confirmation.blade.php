...
@section('title', __('site.register.confirm_title'))
    ...
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        @if (! $registration)
            <div class="text-center py-16">
                ...
                <h1 class="text-2xl font-bold text-gray-900 mb-2">{{ __('site.register.not_found') }}</h1>
                <p class="text-gray-600 mb-6">{{ __('site.register.not_found_sub') }} <strong>{{ $reference }}</strong></p>
                <a href="{{ url('/') }}" class="inline-flex items-center gap-2 bg-brand-blue text-white px-6 py-3 rounded-md font-semibold">{{ __('site.register.return_home') }}</a>
            </div>
        @else
            <div class="text-center space-y-4">
                ...
                <h1 class="text-3xl font-extrabold text-gray-900">{{ __('site.register.complete') }}</h1>
                <p class="text-lg text-gray-600">{{ __('site.register.thank_you', ['name' => $registration->client_name]) }}</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg border-t-4 border-t-brand-gold overflow-hidden">
                <div class="bg-gray-50 border-b border-gray-100 px-8 py-6 text-center">
                    <p class="text-sm font-medium text-gray-500 uppercase tracking-wider">{{ __('site.register.reference') }}</p>
                    <p class="text-3xl font-mono font-bold text-brand-blue mt-2">{{ $registration->reference_number }}</p>
                </div>
                <div class="p-8 space-y-6">
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('site.register.full_name') }}</dt>
                            <dd class="mt-1 font-semibold text-gray-900">{{ $registration->client_name }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('site.register.email') }}</dt>
                            <dd class="mt-1 font-semibold text-gray-900">{{ $registration->client_email }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('site.register.phone') }}</dt>
                            <dd class="mt-1 font-semibold text-gray-900">{{ $registration->client_phone }}</dd>
                        </div>
                        @if ($registration->company_name)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('site.register.company') }}</dt>
                                <dd class="mt-1 font-semibold text-gray-900">{{ $registration->company_name }}</dd>
                            </div>
                        @endif
                    </dl>
                    <div class="pt-4 border-t border-gray-100">
                        <dt class="text-sm font-medium text-gray-500 mb-2">{{ __('site.register.select') }}</dt>
            ...
            <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100">
                <h3 class="text-lg font-bold text-gray-900 mb-4">{{ __('site.register.next') }}</h3>
                <ul class="space-y-4">
                    @foreach ([
                        __('site.register.step1'),
                        __('site.register.step2'),
                        __('site.register.step3'),
                    ] as $i => $step)
                        <li class="flex gap-3">
            ...
            <div class="flex flex-wrap justify-center gap-4">
                @auth('beyond')
                    <a href="{{ route('student.dashboard') }}" class="inline-flex items-center gap-2 bg-brand-blue text-white px-5 py-2.5 rounded-md font-semibold hover:bg-brand-dark">
                        <i data-lucide="layout-dashboard" class="w-4 h-4"></i> {{ __('site.register.portal') }}
                    </a>
                @else
                    <a href="{{ url('/login') }}" class="inline-flex items-center gap-2 bg-brand-blue text-white px-5 py-2.5 rounded-md font-semibold hover:bg-brand-dark">
                        <i data-lucide="log-in" class="w-4 h-4"></i> {{ __('site.register.login_track') }}
                    </a>
                @endauth
